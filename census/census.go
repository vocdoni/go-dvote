...
type Manager struct {
	...
	// TODO(demogat): should we protect Census with the mutex too?
	TreesMu sync.RWMutex
	Trees   map[string]*tree.Tree // MkTrees map of merkle trees indexed by censusId

	Storage data.Storage
	// TODO(demogat): this looks very racy. probably replace it with a chan.
...
func (m *Manager) ImportQueueDaemon() {
	...
	for {
		for cid, uri := range m.ImportQueue {
			// TODO(demogat): this lock is separate from the one
